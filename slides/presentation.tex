% Adapted from UH Manoa theme by:
% UH Manoa presentation theme for beamer
% Jeff Delmerico <jeffdelmerico@gmail.com> 2012
% https://github.com/jeffdelmerico/UH_Beamer_Theme.git


\documentclass{beamer}
%\documentclass[handout]{beamer}
\usetheme{PD}

\usepackage[utf8]{inputenc}
\usepackage[absolute,overlay]{textpos}
\usepackage{helvet}
\usepackage{tikz}
\usepackage{listings,lstautogobble}
\usepackage{syntax}
\usepackage{mathrsfs}
\usetikzlibrary{tikzmark,fit,decorations.pathreplacing}

\title{Spores}
\subtitle{\newline Closures per la programmazione funzionale distribuita e type-safe}
\author{Matteo Di Pirro}
\date{17 Febbraio 2017}
\institute{Università degli Studi di Padova}

\lstset{
	autogobble=true
}

\setlength{\grammarindent}{3em}

\begin{document}

\newcommand{\turnOffNumbers}{true} %hide frame numbers in footer

\begin{frame}[noframenumbering]
\titlepage
\end{frame}

\let\turnOffNumbers\empty
\begin{frame}
	\frametitle{Outline}
	\tableofcontents
\end{frame}

\section{Scenario}
\input{slides/scenario}
\input{slides/passaggioDiFunzioni}
\input{slides/distributedComputing}

\section{Spore}
\input{slides/spore}
\input{slides/sintassi}
\input{slides/proprieta}

\section{Formalizzazione}
\subsection{Semantica operazionale}
\input{slides/semanticaOperazionale}

\subsection{Typing}
\input{slides/typing}
\input{slides/subtyping}

\subsection{Soundness}
\input{slides/soundness}

\subsection{Tipi esclusi}
\input{slides/esclusi}

\section{Conclusioni}
\input{slides/conclusione}
\input{slides/problemiRunTime}

\appendix
\makethanks
\renewcommand{\turnOffNumbers}{true} %hide frame numbers in footer
\input{slides/import}
\end{document}
